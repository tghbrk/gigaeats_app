...
class SupabaseAuthRepository implements AuthRepository {
  ...
  Future<Either<Failure, UserEntity>> signInWithEmailAndPassword({
    ...
    try {
      ...
      if (result.isSuccess && result.user != null) {
        return Right(_mapUserToEntity(result.user!));
      } else {
        return Left(AuthFailure(message: result.errorMessage ?? 'Sign in failed'));
      }
    } catch (e) {
      return Left(AuthFailure(message: 'An unexpected error occurred: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, UserEntity>> signUpWithEmailAndPassword({
    ...
    try {
      final result = await _authService.registerWithEmailAndPassword(
        ...
        role: _mapRoleEntityToModel(role),
      );

      if (result.isSuccess && result.user != null) {
        return Right(_mapUserToEntity(result.user!));
      } else {
        return Left(AuthFailure(message: result.errorMessage ?? 'Sign up failed'));
      }
    } catch (e) {
      return Left(AuthFailure(message: 'An unexpected error occurred: ${e.toString()}'));
    }
  ...
  Future<Either<Failure, void>> sendPasswordResetEmail(String email) async {
    try {
      ...
      if (result.isSuccess) {
        ...
        return Left(AuthFailure(message: result.errorMessage ?? 'Failed to send password reset email'));
      }
    } catch (e) {
      return Left(AuthFailure(message: 'An unexpected error occurred: ${e.toString()}'));
    }
  ...
  Future<Either<Failure, void>> updatePassword({
    ...
    try {
      ...
      if (result.isSuccess) {
        ...
        return Left(AuthFailure(message: result.errorMessage ?? 'Failed to update password'));
      }
    } catch (e) {
      return Left(AuthFailure(message: 'An unexpected error occurred: ${e.toString()}'));
    }
  ...
  Future<Either<Failure, bool>> isAuthenticated() async {
    try {
      return Right(_authService.isAuthenticated);
    } catch (e) {
