...
import '../../../vendors/data/repositories/driver_repository.dart';
import '../../../vendors/presentation/widgets/driver_card.dart';
import '../../../vendors/presentation/widgets/driver_status_indicator.dart';
import '../../../../shared/widgets/loading_widget.dart';
import '../../../../shared/widgets/error_widget.dart';

/// Provider for driver repository (admin access)
final adminDriverRepositoryProvider = Provider<DriverRepository>((ref) {
  return DriverRepository();
});

/// Provider for all drivers (admin access)
final adminDriversProvider = FutureProvider<List<Driver>>((ref) async {
  final driverRepository = ref.read(adminDriverRepositoryProvider);
...
class _AdminFleetManagementScreenState extends ConsumerState<AdminFleetManagementScreen> {
          ...
          itemBuilder: (context, index) {
            ...
            return Padding(
              ...
              child: DriverCard(
                ...
                onStatusChanged: (newStatus) => _updateDriverStatus(driver, newStatus),
              ),
            );
          },
        );
  ...
  Future<void> _updateDriverStatus(Driver driver, DriverStatus newStatus) async {
    try {
      final driverRepository = ref.read(adminDriverRepositoryProvider);
      await driverRepository.updateDriverStatus(driver.id, newStatus);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Driver status updated to ${newStatus.name}')),
        );
      }
    } catch (e) {
      if (mounted) {
