...
class CrossRoleProvisioningService extends BaseRepository {
  ...
  Future<DriverInvitationResult> createDriverInvitation({
    ...
    try {
      final response = await supabase.rpc('create_driver_invitation', params: {
  ...
  Future<DriverInvitationValidation> validateDriverInvitationToken(String token) async {
    try {
      final response = await supabase.rpc('validate_driver_invitation_token', params: {
        'p_token': token,
      });
  ...
  Future<DriverAccountCreationResult> createDriverAccountFromInvitation({
    ...
    try {
      final response = await supabase.rpc('create_driver_account_from_invitation', params: {
  ...
  Future<RoleTransitionResult> requestRoleTransition({
    ...
    try {
      final response = await supabase.rpc('request_role_transition', params: {
        'p_user_id': userId,
  ...
  Future<RoleTransitionResult> approveRoleTransition({
    ...
    try {
      final response = await supabase.rpc('approve_role_transition', params: {
  ...
  Future<AccountProvisioningStats> getAccountProvisioningStats() async {
    try {
      final response = await supabase.rpc('get_account_provisioning_stats');
      final result = response.first;
