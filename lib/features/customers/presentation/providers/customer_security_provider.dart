...
class CustomerSecurityState {
  ...
  final CustomerSecurity? security;
...
class CustomerSecurityNotifier extends StateNotifier<CustomerSecurityState> {
  ...
  Future<List<SecurityAuditLog>> checkSuspiciousActivity() async {
    try {
      final authState = _ref.read(authStateProvider);
      final user = authState.user;
  ...
  bool shouldAutoLock() {
    ...
    final timeoutMinutes = state.security!.autoLockTimeoutMinutes;
...
final securitySettingsProvider = FutureProvider<CustomerSecurity?>((ref) async {
  ...
  final securityService = ref.watch(customerSecurityServiceProvider);
  return securityService.getSecuritySettings(user.id);
});
