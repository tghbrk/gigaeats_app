...
class SecurityScoreCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final score = security.securityScore;
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        security.securityLevelDescription,
...
class BiometricAuthenticationCard extends ConsumerWidget {
  ...
  void _toggleBiometric(WidgetRef ref, bool enabled) async {
    try {
      if (enabled) {
          ...
          children: [
            Row(
              children: [
                ...
                Switch(
                  value: security?.pinEnabled ?? false,
                  onChanged: (value) => _togglePIN(context, ref, value),
                  activeColor: AppTheme.primaryColor,
                ),
              ],
            ),
            if (security?.pinEnabled == true) ...[
              const SizedBox(height: 12),
              Row(
                children: [
                  Expanded(
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        security?.transactionPinRequired == true 
                            ? 'PIN required for transactions above RM ${security!.largeTransactionThreshold.toStringAsFixed(0)}'
                ...
                Switch(
                  value: security?.transactionPinRequired ?? false,
                  onChanged: (value) => _toggleTransactionPIN(context, ref, value),
                  activeColor: AppTheme.primaryColor,
                ),
              ],
            ),
            if (security?.transactionPinRequired == true) ...[
              const SizedBox(height: 12),
              OutlinedButton.icon(
          ...
          children: [
            Row(
              children: [
                    ...
                    children: [
                      ...
                      Text(
                        security?.autoLockEnabled == true
                            ? 'Lock after ${security!.autoLockTimeoutMinutes} minutes of inactivity'
                ...
                Switch(
                  value: security?.autoLockEnabled ?? false,
                  onChanged: (value) => _toggleAutoLock(context, ref, value),
                  activeColor: AppTheme.primaryColor,
                ),
              ],
            ),
            if (security?.autoLockEnabled == true) ...[
              const SizedBox(height: 12),
              OutlinedButton.icon(
...
class MFASettingsCard extends ConsumerWidget {
  ...
  void _toggleMFA(WidgetRef ref, bool enabled) async {
    try {
      if (enabled) {
...
class SecurityAuditLogCard extends StatelessWidget {
  ...
  Color _getRiskColor(SecurityRiskLevel riskLevel) {
    switch (riskLevel) {
  ...
  IconData _getEventIcon(SecurityEventType eventType) {
    switch (eventType) {
