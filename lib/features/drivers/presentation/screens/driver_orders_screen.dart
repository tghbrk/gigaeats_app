...
class DriverOrdersScreen extends ConsumerWidget {
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return DriverOrderCard(
            ...
            isActive: isActive,
            isHistory: isHistory,
            // Distance calculation will be handled inside the card widget
            // using FutureBuilder for better performance
            distanceToPickup: null,
            distanceToDelivery: null,
            onAccept: isAvailable ? () => _acceptOrder(context, ref, order.id) : null,
            onReject: isAvailable ? () => _rejectOrder(context, ref, order.id) : null,
            onViewDetails: () => _viewOrderDetails(context, order.id),
            onStartDelivery: isActive ? () => _startDelivery(context, order.id) : null,
            onUpdateStatus: null, // Update Status button removed from Active tab
            onViewReceipt: isHistory ? () => _viewReceipt(context, order) : null,
          );
        },
      ),
    );
  ...
  void _acceptOrder(BuildContext context, WidgetRef ref, String orderId) async {
  ...
  void _rejectOrder(BuildContext context, WidgetRef ref, String orderId) async {
  ...
  void _viewReceipt(BuildContext context, DriverOrder order) {
  ...
  void _showReceiptDialog(BuildContext context, DriverOrder order) {
    showDialog(
      context: context,
