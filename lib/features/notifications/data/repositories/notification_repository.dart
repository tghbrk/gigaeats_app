...
class NotificationRepository extends BaseRepository {
  ...
  Future<NotificationCounts> getNotificationCounts({String? userId}) async {
    try {
      final response = await supabase.rpc('get_user_notification_counts', params: {
        'p_user_id': userId,
      });
  ...
  Future<bool> markAsRead(String notificationId, {String? userId}) async {
    try {
      final response = await supabase.rpc('mark_notification_read', params: {
  ...
  Future<int> markAllAsRead({String? userId}) async {
    try {
      final response = await supabase.rpc('mark_all_notifications_read', params: {
        'p_user_id': userId,
      });
  ...
  Future<String?> createNotificationFromTemplate({
    ...
    try {
      final response = await supabase.rpc('create_notification_from_template', params: {
  ...
  Future<int> cleanupExpiredNotifications() async {
    try {
      final response = await supabase.rpc('cleanup_expired_notifications');
      return response ?? 0;
    } catch (e) {
