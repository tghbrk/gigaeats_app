...
/// Provider for driver repository
...
class _DriverManagementScreenState extends ConsumerState<DriverManagementScreen> {
  ...
  void _updateDriverStatus(Driver driver, DriverStatus newStatus) async {
    try {
      final driverRepository = ref.read(driverRepositoryProvider);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${driver.name} status updated to ${newStatus.displayName}'),
