...
class DeepLinkService {
  ...
  static Future<void> _handleVerificationError(Map<String, String> params, WidgetRef ref) async {
    ...
    final email = params['email'] ?? ref.read(enhancedAuthStateProvider).pendingVerificationEmail;
  ...
  static Future<void> _handleEmailVerificationSuccess(WidgetRef ref) async {
    ...
    try {
      // Handle email verification completion using enhanced auth provider
      final autoLoginSuccessful = await ref.read(enhancedAuthStateProvider.notifier).handleEmailVerificationComplete();
      ...
      if (autoLoginSuccessful && authState.user != null) {
        ...
        // Email verification successful with auto-login
        debugPrint('✅ DeepLinkService: Email verification and auto-login successful');
      } else {
        debugPrint('ℹ️ DeepLinkService: Auto-login not available, using pending email: $email');
        // Email verification successful but manual login required
      }
      ...
      // Handle error case - enhanced auth provider manages its own state
