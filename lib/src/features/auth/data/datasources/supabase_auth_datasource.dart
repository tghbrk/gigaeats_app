...
import '../../../user_management/domain/user.dart' as app_models;
...
class SupabaseAuthDataSourceImpl implements SupabaseAuthDataSource {
  ...
  Future<app_models.User> signInWithEmailAndPassword({
    ...
    try {
      ...
      debugPrint('🔐 SupabaseAuthDatasource: Attempting to sign in user: $email');

      final response = await _supabaseClient.auth.signInWithPassword(
        email: email,
        password: password,
      ).timeout(const Duration(seconds: 15), onTimeout: () {
        debugPrint('❌ SupabaseAuthDatasource: Supabase auth call timed out after 15 seconds');
        throw Exception('Authentication timeout after 15 seconds. Please check your connection and try again.');
      });

      debugPrint('🔐 SupabaseAuthDatasource: Supabase auth call completed');

      if (response.user == null) {
        debugPrint('❌ SupabaseAuthDatasource: No user in response');
        throw const app_exceptions.AuthException(message: 'Failed to sign in user');
      }

      debugPrint('✅ SupabaseAuthDatasource: Supabase auth successful, user ID: ${response.user!.id}');
      debugPrint('🔍 SupabaseAuthDatasource: About to fetch user profile...');

      // Get user profile from database
      final userProfile = await _getUserProfile(response.user!.id);

      _logger.info('✅ User signed in successfully: $email');
      debugPrint('✅ SupabaseAuthDatasource: Complete sign-in successful for: $email');
      return userProfile;
    } on AuthException catch (e) {
      _logger.error('❌ Supabase auth error during sign in: $e', e);
      debugPrint('❌ SupabaseAuthDatasource: Supabase auth error: $e');
      ...
      _logger.error('❌ Unexpected error during sign in: $e', e);
      debugPrint('❌ SupabaseAuthDatasource: Unexpected error: $e');
      throw app_exceptions.AuthException(
  ...
  Future<app_models.User> _getUserProfile(String supabaseUserId) async {
    try {
      _logger.info('🔍 Starting user profile fetch for: $supabaseUserId');
      debugPrint('🔍 SupabaseAuthDatasource: Starting user profile fetch for: $supabaseUserId');
      ...
      final response = await _supabaseClient
          ...
          .timeout(const Duration(seconds: 10), onTimeout: () {
            debugPrint('❌ SupabaseAuthDatasource: User profile fetch timed out after 10 seconds');
            throw Exception('User profile fetch timeout after 10 seconds');
          });

      _logger.info('✅ User profile fetched successfully');
      debugPrint('✅ SupabaseAuthDatasource: User profile fetched successfully');
      return app_models.User.fromJson(response);
    } catch (e) {
      _logger.error('❌ Failed to get user profile from Supabase: $e', e);
      debugPrint('❌ SupabaseAuthDatasource: Failed to get user profile: $e');
