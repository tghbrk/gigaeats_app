...
class CustomerWalletTopupService {
  ...
  Future<Map<String, dynamic>> createPaymentIntent({
    ...
    try {
      ...
      final paymentIntent = data['payment_intent'] as Map<String, dynamic>;
  ...
  /// Create a payment intent for wallet top-up with saved payment method
  Future<Map<String, dynamic>> createPaymentIntentWithSavedMethod({
    required double amount,
    required String paymentMethodId,
    String currency = 'myr',
  }) async {
    try {
      debugPrint('🔍 [WALLET-TOPUP-SERVICE] Creating payment intent with saved method for RM $amount');
      ...
      // Call the wallet-topup Edge Function with saved payment method
      final response = await _client.functions.invoke(
        'wallet-topup',
        body: {
          'amount': amount,
      ...
      debugPrint('✅ [WALLET-TOPUP-SERVICE] Payment intent with saved method created successfully');
      ...
      debugPrint('❌ [WALLET-TOPUP-SERVICE] Service error with saved method: $e');
