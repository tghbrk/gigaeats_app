...
/// Exception thrown when wallet has insufficient balance for payment
...
class CustomerOrderService {
  ...
  Future<Order> createOrderFromCart({
    ...
    try {
      ...
      _logger.info('CustomerOrderService: Cart has ${cartState.items.length} items');
      _logger.info('CustomerOrderService: Cart isEmpty: ${cartState.isEmpty}');

      // Validate cart state
      if (cartState.isEmpty) {
        _logger.error('CustomerOrderService: Cart is empty - cannot create order');
        throw Exception('Cart is empty');
      }

      if (cartState.deliveryMethod.requiresDriver && cartState.selectedAddress == null) {
        _logger.error('CustomerOrderService: Delivery address required but not provided');
      ...
      if (user == null) {
        _logger.error('CustomerOrderService: User not authenticated');
        throw Exception('User not authenticated');
      }
      _logger.info('CustomerOrderService: User authenticated: ${user.id}');
      ...
      _logger.info('CustomerOrderService: Customer name: $customerName, phone: $customerPhone');

      // Convert cart items to order items
      _logger.info('CustomerOrderService: Converting ${cartState.items.length} cart items to order items');
      ...
      _logger.info('CustomerOrderService: Created ${orderItems.length} order items');

      // Get vendor information from first item
      final firstItem = cartState.items.first;
      final vendorId = firstItem.vendorId;
      _logger.info('CustomerOrderService: Vendor ID from first item: $vendorId');
      ...
      // Validate all required fields before creating orderData
      _logger.info('CustomerOrderService: Validating required fields before orderData creation');
      _logger.info('CustomerOrderService: vendorId: $vendorId');
      _logger.info('CustomerOrderService: user.id: ${user.id}');
      _logger.info('CustomerOrderService: orderItems.length: ${orderItems.length}');
      _logger.info('CustomerOrderService: deliveryDateTime: $deliveryDateTime');
      _logger.info('CustomerOrderService: paymentMethod: $paymentMethod');

      if (vendorId.isEmpty) {
        _logger.error('CustomerOrderService: Vendor ID is empty');
        throw Exception('Vendor ID is required but not found in cart');
      }

      if (orderItems.isEmpty) {
        _logger.error('CustomerOrderService: Order items list is empty');
        throw Exception('Order items are required but cart conversion resulted in empty list');
      }
      ...
      _logger.info('CustomerOrderService: Payment method being sent: $paymentMethod');
  ...
  Future<Map<String, dynamic>> _processWalletPayment({
    ...
    try {
      ...
      if (result['success'] != true) {
        ...
        final errorMessage = result['error'] ?? result['message'] ?? 'Wallet payment failed';

        if (errorCode == 'INSUFFICIENT_FUNDS') {
          // Create a structured error for insufficient balance
          throw InsufficientBalanceException(
            message: errorMessage,
            availableBalance: result['available_balance']?.toDouble() ?? 0.0,
  ...
  Map<String, dynamic> _categorizeWalletPaymentError(dynamic error, String orderId) {
    // Handle specific insufficient balance exception
    if (error is InsufficientBalanceException) {
      return {
        'success': false,
        'payment_method': 'wallet',
        'status': 'failed',
        'error': 'Insufficient wallet balance. You need RM ${error.shortfall.toStringAsFixed(2)} more to complete this order.',
  ...
  Future<List<Order>> getCustomerOrders(String customerId, {int limit = 200}) async {
    try {
      ...
      for (int i = 0; i < response.length && i < 3; i++) {
        ...
        _logger.info('CustomerOrderService: Order ${i + 1} - order_items type: ${orderItems.runtimeType}');
        _logger.info('CustomerOrderService: Order ${i + 1} - order_items length: ${orderItems is List ? orderItems.length : 'N/A'}');
        if (orderItems is List && orderItems.isNotEmpty) {
          _logger.info('CustomerOrderService: Order ${i + 1} - First item: ${orderItems.first}');
        }
      }
      ...
      for (int i = 0; i < response.length; i++) {
        ...
        try {
          ...
          if (orderData['order_items'] != null && orderData['order_items'] is List) {
            ...
            _logger.info('CustomerOrderService: Processing order ${i + 1} with ${orderItems.length} items');
            ...
            _logger.warning('CustomerOrderService: No order_items found for order ${i + 1}');
          }
          ...
          _logger.info('CustomerOrderService: About to parse order ${i + 1} with data keys: ${orderData.keys}');
