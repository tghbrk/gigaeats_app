...
class CartState {
  ...
  double get deliveryFee {
    ...
    switch (deliveryMethod) {
      // TODO: Restore when lalamove delivery method is implemented in DeliveryMethod enum
      ...
      default:
        return 0.0; // Default no delivery fee for unhandled delivery methods
    }
  }
...
class CartNotifier extends StateNotifier<CartState> {
  ...
  void addItem({
    ...
    _logger.info('🛒 [BASE-CART] Adding product to cart: ${product.name} (qty: $quantity) from ${vendor.businessName}');
    ...
    if (existingItemIndex >= 0) {
      ...
      _logger.info('✅ [BASE-CART] Updated existing item quantity. Cart now has ${state.totalItems} items');
    } else {
      // Add new item
      ...
      _logger.info('✅ [BASE-CART] Added new item to cart. Cart now has ${state.totalItems} items');
    }
  }
