...
class VendorRepository extends BaseRepository {
  ...
  Future<List<Vendor>> getVendors({
    ...
    return executeQuery(() async {
      debugPrint('🏪 [VENDOR-REPOSITORY] ===== getVendors CALLED =====');
      debugPrint('🏪 [VENDOR-REPOSITORY] Parameters:');
      debugPrint('  - Search query: $searchQuery');
      debugPrint('  - Cuisine types: $cuisineTypes');
      debugPrint('  - Min rating: $minRating');
      debugPrint('  - Halal only: $isHalalOnly');
      ...
      debugPrint('🏪 [VENDOR-REPOSITORY] Got authenticated client');
      ...
      debugPrint('🏪 [VENDOR-REPOSITORY] Base query created with is_active=true filter');

      // Apply search filter
      if (searchQuery != null && searchQuery.isNotEmpty) {
        debugPrint('🏪 [VENDOR-REPOSITORY] Applying search filter: $searchQuery');
        query = query.or(
      ...
      if (cuisineTypes != null && cuisineTypes.isNotEmpty) {
        debugPrint('🏪 [VENDOR-REPOSITORY] Applying cuisine filter: $cuisineTypes');
        query = query.overlaps('cuisine_types', cuisineTypes);
      }

      // Apply rating filter
      if (minRating != null) {
        debugPrint('🏪 [VENDOR-REPOSITORY] Applying rating filter: >= $minRating');
        query = query.gte('rating', minRating);
      }

      // Apply halal filter
      if (isHalalOnly == true) {
        debugPrint('🏪 [VENDOR-REPOSITORY] Applying halal filter: true');
        query = query.eq('is_halal_certified', true);
      }

      debugPrint('🏪 [VENDOR-REPOSITORY] Executing query with ordering and pagination...');
      // Apply ordering and pagination after filters
      final response = await query
          .order('rating', ascending: false)
          .range(offset, offset + limit - 1);

      debugPrint('🏪 [VENDOR-REPOSITORY] Query executed. Raw response length: ${response.length}');

      // Handle empty results gracefully
      if (response.isEmpty) {
        debugPrint('❌ [VENDOR-REPOSITORY] No vendors found with current filters');
        return [];
      }

      debugPrint('🏪 [VENDOR-REPOSITORY] Processing ${response.length} vendor records...');

      final vendors = response.map((json) {
        try {
          debugPrint('🏪 [VENDOR-REPOSITORY] Processing vendor: ${json['business_name']} (ID: ${json['id']})');
          ...
          final vendor = Vendor.fromJson(processedJson);
          debugPrint('✅ [VENDOR-REPOSITORY] Successfully parsed vendor: ${vendor.businessName}');
          return vendor;
        } catch (e) {
          debugPrint('❌ [VENDOR-REPOSITORY] Error parsing vendor JSON: $e');
          debugPrint('❌ [VENDOR-REPOSITORY] JSON data: $json');
          rethrow;
        }
      }).toList();

      debugPrint('✅ [VENDOR-REPOSITORY] Successfully processed ${vendors.length} vendors');
      debugPrint('🏪 [VENDOR-REPOSITORY] Vendor names: ${vendors.map((v) => v.businessName).join(', ')}');
  ...
  Stream<List<Vendor>> getVendorsStream({
    ...
    return executeStreamQuery(() {
      ...
      // TODO: Implement location filtering when service_areas table is available
