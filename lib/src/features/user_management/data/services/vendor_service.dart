...
class VendorService {
  ...
  Future<List<Vendor>> getVendors({
    ...
    _logger.info('🏪 [VENDOR-SERVICE] Getting vendors with parameters:');
    _logger.info('  - Search query: $searchQuery');
    _logger.info('  - Cuisine types: $cuisineTypes');
    _logger.info('  - Min rating: $minRating');
    _logger.info('  - Halal only: $isHalalOnly');
    _logger.info('  - Max distance: $maxDistance');
    _logger.info('  - Location: ($latitude, $longitude)');
    ...
    try {
      ...
      _logger.info('✅ [VENDOR-SERVICE] Repository returned ${vendors.length} vendors');
      return vendors;
    } catch (e) {
      _logger.error('❌ [VENDOR-SERVICE] Error getting vendors from repository', e);
      rethrow;
    }
  }
