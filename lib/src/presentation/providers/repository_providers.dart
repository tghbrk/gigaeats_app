...
final vendorAnalyticsProvider = FutureProvider.family<Map<String, dynamic>, Map<String, DateTime?>>((ref, dateRange) async {
  final vendor = await ref.watch(currentVendorProvider.future);
...
final vendorSalesBreakdownProvider = FutureProvider.family<Map<String, dynamic>, Map<String, DateTime?>>((ref, dateRange) async {
  final vendor = await ref.watch(currentVendorProvider.future);
...
final vendorCategoryPerformanceProvider = FutureProvider.family<Map<String, dynamic>, Map<String, DateTime?>>((ref, dateRange) async {
  final vendor = await ref.watch(currentVendorProvider.future);
...
final vendorSalesSummaryProvider = FutureProvider.family<Map<String, dynamic>, Map<String, DateTime?>>((ref, dateRange) async {
  ...
  // TODO: Restore vendorRepository when sales data methods are available - commented out for analyzer cleanup
  // final vendorRepository = ref.watch(vendorRepositoryProvider);

  // Get current period sales breakdown to calculate total
  // TODO: Restore currentSales when fold method is available - commented out for analyzer cleanup
  ...
  final totalSales = 0.0; // currentSales.fold<double>(0.0, (sum, item) => sum + (item['total_sales'] ?? 0.0));
  final totalOrders = 0; // currentSales.fold<int>(0, (sum, item) => sum + ((item['total_orders'] ?? 0) as int));
  ...
  if (currentStart != null && currentEnd != null) {
    // TODO: Restore periodDuration when previous period calculation is needed - commented out for analyzer cleanup
    // final periodDuration = currentEnd.difference(currentStart);
    // TODO: Restore previous period calculation when sales data is available - commented out for analyzer cleanup
    // final previousStart = currentStart.subtract(periodDuration);
    // final previousEnd = currentStart;

    // TODO: Restore previousSales when fold method is available - commented out for analyzer cleanup
    ...
    final previousTotalSales = 0.0; // previousSales.fold<double>(0.0, (sum, item) => sum + (item['total_sales'] ?? 0.0));
...
final vendorNotificationsProvider = FutureProvider.family<List<Map<String, dynamic>>, bool?>((ref, unreadOnly) async {
  ...
  final vendorRepository = ref.watch(vendorRepositoryProvider);
  // TODO: Restore unreadOnly parameter when supported - commented out for analyzer cleanup
...
final salesAgentServiceProvider = Provider<SalesAgentService>((ref) {
  ...
  // return SalesAgentService(salesAgentRepository);
  return SalesAgentService(); // Placeholder
});
...
final platformVendorsProvider = FutureProvider<List<Vendor>>((ref) async {
  if (kIsWeb) {
    ...
    final vendorsState = <String, dynamic>{'vendors': <dynamic>[]}; // Placeholder
