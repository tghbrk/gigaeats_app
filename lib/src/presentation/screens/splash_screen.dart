...
class _SplashScreenState extends ConsumerState<SplashScreen>
  ...
  static const int _maxAuthCheckAttempts = 5;
  ...
  Future<void> _checkAuthenticationStatus() async {
    ...
    try {
      ...
      if (authState.status == AuthStatus.authenticated && authState.user != null) {
        ...
        _authCheckAttempts++;
        debugPrint('SplashScreen: Auth state still loading (${authState.status}), attempt $_authCheckAttempts/$_maxAuthCheckAttempts');

        if (_authCheckAttempts >= _maxAuthCheckAttempts) {
          debugPrint('SplashScreen: Max auth check attempts reached, navigating to login');
          if (mounted) {
            context.go(AppRoutes.login);
          }
        } else {
